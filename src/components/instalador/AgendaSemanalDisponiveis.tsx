...
import { AgendaServicoDisponivel } from "./AgendaServicoDisponivel";
...
interface AgendaSemanalDisponiveisProps {
  servicos: Servico[];
  certificacoes: string[];
  onSolicitar: (id: string) => void;
  solicitandoId: string | null;
}

export function AgendaSemanalDisponiveis({ 
  servicos, 
  certificacoes,
  onSolicitar, 
  solicitandoId 
}: AgendaSemanalDisponiveisProps) {
  ...
  // Contar total de serviços na semana
  const totalServicosSemana = servicosPorDia.reduce((acc, dia) => acc + dia.servicos.length, 0);
    ...
    return (
      <div className="space-y-4">
            ...
            <button
              ...
              className={cn(
                ...
                diaSelecionadoMobile && isSameDay(diaSelecionadoMobile, data) 
                  ...
                  : "bg-card",
                servicosDia.length > 0 && !diaSelecionadoMobile?.getTime() && "bg-blue-50 border-blue-200"
              )}
            >
              <p className={cn(
                "text-[10px] uppercase",
        ...
        <div className="space-y-2">
            ...
            <>
              ...
              {servicosPorDia
                ...
                ?.servicos.map(servico => (
                  <AgendaServicoDisponivel
                    key={servico.id}
                    servico={servico}
                    temCertificacao={temCertificacaoParaServico(servico.tipo_servico || [])}
                    onSolicitar={onSolicitar}
                    isLoading={solicitandoId === servico.id}
                  />
                ))}
          ...
          {!diaSelecionadoMobile && totalServicosSemana === 0 && (
            <div className="text-center py-8 text-muted-foreground bg-card rounded-lg">
              <Calendar className="h-12 w-12 mx-auto mb-2 opacity-50" />
              <p>Nenhum serviço disponível esta semana</p>
            </div>
          )}
        </div>
      </div>
        ...
        {servicosPorDia.map(({ data, servicos: servicosDia }) => (
          <div 
            ...
            className={cn(
              ...
              servicosDia.length > 0 && "border-blue-200"
            )}
          >
            {/* Header do dia */}
            <div className={cn(
              ...
              servicosDia.length > 0 && !isToday(data) && "bg-blue-50"
            )}>
              ...
              {servicosDia.length > 0 && (
                <p className="text-[10px] text-blue-600 mt-1">
                  {servicosDia.length} disponíve{servicosDia.length > 1 ? "is" : "l"}
                </p>
              )}
            </div>
